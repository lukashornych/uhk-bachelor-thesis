...
\projectinfo{
  ...
  % todo
  keywords.cs={web, technologie, server, klient, backend, frontend, server-side, client-side, databáze, fulltext}, % klíčová slova v českém či slovenském jazyce / keywords in czech or slovak language
  keywords.en={web, technologies, server, client, backend, frontend, server-side, client-side, database, fulltext}, % klíčová slova v anglickém jazyce / keywords in english
  % Anotace
  annotation.cs={
    Bakalářská práce se zabývá návrhem a implementací webové aplikace fungující na všech standardních zařízení
    ...
    Součástí návrhu je analýza dostupných webových technologií a nástrojů pro vývoj webové aplikace od nuly až po
    běh v produkčním prostředí.
    Práce následně vybírá nejvhodnější technologie a nástroje pro samotnou implementaci moderní webové aplikace
    umožňující vytváření a prohlížení profilových karet, která se svým chováním přibližuje nativním desktopovým a mobilním aplikacím.
  },
