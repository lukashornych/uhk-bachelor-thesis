...
Následně je již možné začít se samotnou implementací s pomocí vybraných technologií a prototypu \ac{UI}.
	...
	mezer a prvků a základní pravidla, jako jsou pozicování tlačítek, rozvržení seznamů, modální okna a podobně.
		...
		\cntcapfigure{rovnovaha_paka}{10cm}{Páka rovnováhy vzhledu.}{\cite{vizualni_rovnovaha}}
		...
		\cntcapfigure{paleta_barev}{\linewidth}{Navržená paleta barev.}{[autor]}
		...
		\cntcapfigure{blok_obsahu}{8cm}{Ukázka zaoblených rohů.}{[autor]}
		...
		\cntcapfigure{ukazka_vyberu_1}{6cm}{Ukázka signalizace výběru v navigaci.}{[autor]}

		\cntcapfigure{ukazka_vyberu_2}{4cm}{Ukázka signalizace výběru konkrétního prvku.}{[autor]}
		...
		rozvržení stránek.
			...
			\cntcapfigure{modalni_okno_informacni_ukazka}{10cm}{Ukázka konkrétního informačního modální okna.}{[autor]}

			\cntcapfigure{modalni_okno_akcni_ukazka}{10cm}{Ukázka konkrétního akčního modálního okna.}{[autor]}
		...
		Po prvcích strukturující obsah je možné navrhnout prvky, se kterými bude uživatel přímo či nepřímo interagovat.
			...
			\cntcapfigure{textove_pole}{8cm}{Ukázka textového pole s ikonou a nápovědou.}{[autor]}
			...
			\cntcapfigure{pole_lokace}{8cm}{Ukázka pole pro výběr geografického bodu v mapě.}{[autor]}
			...
			\cntcapfigure{obrazkove_pole}{5cm}{Ukázka pole s nahraným obrázkem.}{[autor]}
...
https://{userPart}.example.com
		...
		mapovat na \ac{JSON} dokumenty. \cite{restfulapi}
			...
			\cntcapfigure{api_model_uloziste}{8cm}{Stromový přehled struktury API uložiště.}{[autor]}

			Pro přehlednost vyhledávání karet a míst existuje separátní vyhledávací \ac{API}.

			\cntcapfigure{api_model_vyhledavani}{8cm}{Stromový přehled struktury API vyhledávání karet.}{[autor]}
...
$http.onRequest(async (request) => {
	if (!csrfAllowedMethods.includes(request.method)) {
		...
		\begin{lstlisting}[caption={Ukázka implementace vlastního validátoru využívající API k validaci hodnoty. Zdroj: [fsdf]}]
