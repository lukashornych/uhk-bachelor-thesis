...
Následně je již možné začít se samotnou implementací s pomocí vybraných technologií a prototypu \ac{UI}.
	...
	\subsection{Implementace API serveru}
		...
		\subseciton{Komunikace s databázových systémem}
...
public class CardLinkTemplate {
    ...
    /* ... zbytek kódu byl vynechán pro přehlednost ... */
		...
		Ukázka možných nastavení pro každou šablonu sítí. % TODO
			...
			Sekvence \lstlisting{userPart} může obsahovat stejné znaky jako \lstlisting{*} sekvence, avšak textové
...
https://pepa-pavel.example.com
			\end{lstlisting}
			Ukázka šablon URL adres a jejich vyhovujících konkrétních URL adres. % todo
...
<button>
	<!-- klíč textu, který při vykreslení bude nahrazen konkrétním textem -->
	{{ $t('general.button.apply') }}
		...
		\begin{lstlisting}[language=JavaScript]
// načtení univerzálních textů v požadovaném JSON formátu. Tyto texty jsou stejné pro všechny jazyky.
import cardLinkTemplates from '../universal/card-link-templates.json'

// načtení běžných překladových textů v požadovaném JSON formátu
import lang from './en/en.json'

// načtení HTML dokumentů s texty pro informační stránky
...
// doplnění načtených textů to hlavního JSON dokumentu
...
// vystavení výsledného sestaveného JSON dokumentu
export default lang
		\end{lstlisting}
...
export const displayId = function (cardId) {
	return async function (displayId) {
		if (!helpers.req(displayId)) {
		    return true
		}

		if (!displayIdFormat(displayId)) {
		    return true
		}

		// provolání API s URL adresou pro ověření, API vrátí zda je validní či nikoliv
...
submitForm () {
	...
	if (!this.$v.$invalid) {
		/* ... formulář je validní, formulář je možné odeslat ... */
	}
}
		\end{lstlisting}
		...
		\begin{lstlisting}[language=JavaScript]
// obecná metoda vystavená knihovnou
this.$notify({
    type: 'success', // nebo error nebo info
    title: this.$t('general.success.done')
})

// pomocné implementované metody
...
this.$notifyError()
		\end{lstlisting}
		Porovnání obecné metody pro vystavení upozornění a pomocných metod. % todo
...
# vytvoří adresář pro aplikaci
RUN mkdir -p /usr/src/netreachme-web-app
WORKDIR /usr/src/netreachme-web-app

# nainstaluje potřebné systémové aplikace
RUN apk update && apk upgrade
RUN apk add git

# sestaví aplikaci do vytvořeného adresáře
COPY . /usr/src/netreachme-web-app/
RUN npm install yarn
RUN yarn install
RUN yarn build

# vystaví aplikaci na portu 3000
EXPOSE 3000
	...
	template:
		...
		spec:
			...
			- image: cloud.canister.io:5000/quickcards/quickcard-web-app:1.0.0
				...
				ports:
					- containerPort: 3000
# ... zbytek kódu vynechán pro přehlednost ... #
		\end{lstlisting}
