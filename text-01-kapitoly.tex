...
Vývoj začal analýzou podobných webových aplikace a zjištěním, co jaká aplikace svým uživatelům poskytuje, a jak.
Následoval návrh vlastní webové aplikace, která kombinuje prvky vícero konkurenčních aplikací a vlastních prvků do jednoho balíčku.
Mezi ty hlavní patří tvorba veřejně dostupných vstupních stránek s kontaktními údaji, odkazy na sociální sítě, firemními hierarchiemi nebo geolokacemi,
fulltextovým vyhledáváním a ukládáním oblíbených stránek.
Nedílnou součástí návrhu bylo i vymyšlení, jak se bude pracovat s uživatelskými účty, a jak by mohl být projekt později
financován.
Bylo zvolena, že uživateslké účtu budou sloužit pouze pro definování vlastníků karet a ukládání cizích karet do oblíbených.
Z možných typů financování (prodej produktů, zobrazování reklama, prémiové plány) byly vybrány právě prémiové plány.
...
Následovala analýza a porovnání webových technologií a nástrojů vhodných pro samotnou implementaci.
...
Pro vyhledávání těchto dat byla zvolena databáze Elasticsearch díky jeho rozšířenosti a možnosti provozu na lokálním stroji.

Souboré úložiště ani systém pro odesílní emailů nebyly implementovány ručně, místo toho byly zvoleny externí služby.
V případě souborového úložiště se jedná Cloudinary, a případě emailů byl zvolen SendinBlue.
...
Po zvolení všech potřebných technologií, byla v první řadě zvolena architektura aplikace, od které se odvíjel výběr technologií a vývoj.
Pro zajištění vysoké interaktivnosti byla zvolena architektura rozdělení na front-endovou \ac{JS} aplikace a back-endový
\ac{API} server.
Pro komunikaci v této architektuře byl zvolen vzor \ac{REST} pro jeho rozšířenost a vyspělost.
Následoval návrh uživatelského rozhraní, podle kterého byl navržen datový model a zvolen návrhový vzor \ac{DDD} pro strukturování
kódu aplikace.
Na základě modelu byla nejdříve implementována business logika, jenž byla v zápětí obalena \ac{REST} \ac{API}.
Business logika zahrnovala mimo jiné implementaci přístupu k databázi pomocí knihovny MyBatis, validaci entit, napojení
na souborý systém, propojení aplikačních událostí s emailových systémem, dále pak vyhledávání karet a míst, konfiguraci
sociálních sítí nebo prémiové plány.
Implementaci \ac{API} zase zahrnovala jeho návrh, zabezpěčení a transformaci doménových entity na přenosné datové objekty.
S připraveným funkčním \ac{API} se zaměření přesunulo tvorbu front-endové části podle návrhu uživatelského rozhraní.
