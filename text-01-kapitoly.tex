...
V první řadě bude nezbytné zjistit, jaké informace bude vhodné umožňovat uživatelům zobrazit na jejich
...
ty budou nicméně hrát roli pouze v případě, jestliže nebude možné rozhodnout čistě podle výhod nebo požadavků.
...
Dostupné druhy informací, které lze definovat na takových stránkách, nejsou ale jasně definované, spíše jsou
...
údajů.
Tyto stránky by se též daly popsat jako digitální rozšířené vizitky, a nejen pro firmy.

Pro tvorbu takovýchto stránek existuje nepřeberné množství webových aplikací, v nichž lze
tyto prezentace jednoduše tvořit a rovnou publikovat.
Ne všechny ale nabízí všechny zmíněné možnosti, a je tak na uživateli, jakou webovou aplikaci zvolí.
...
Existuje spousta webových aplikací, které by se daly považovat za řešení popsané problematiky.
...
Některé navíc neřeší ani ukládání cizích stránek do oblíbených s možností kategorizace či poznámek.
...
Mezi současně nejznámější existující webové aplikace řešící podobné problémy patří například následující.
	...
	Není ani jistota, že jednotlivé odkazy směřují opravdu na skutečné sociální sítě, protože si uživatelé mohou ke každému
	...
	Oproti Linktree obsahuje ale fulltextové vyhledávání a možnost sledování ostatních uživatelů.
	...
	Ta nabízí velice podobné funkce jako Linktree - pouze sdílení odkazů na sociální sítě.
...
většinu požadavků soukromých osob, firem a objektů.
	...
	Protože vyplnění takové karty může být zdlouhavý proces, i za předpokladu intuitivního \firstac{UI}, je potřeba umožnit
	...
	Veřejná karta bude mít jednoduchou zapamatovatelnou \firstac{URL} adresu a bude ji možné vyhledávat pomocí fulltextového vyhledávače.
	...
	\nlparagraph{Kategorizační štítky}

	\noindent Kategorizační štítky budou sloužit pro globální kategorizaci karet, aby měl uživatel povědomí, o jakou kartu se jedná
	...
	\noindent Popisek bude sloužit především pro stručnou definici obsahu karty, tedy co daná karta reprezentuje.
	...
	\nlparagraph{Základní kontaktní údaje}

	\noindent Základními kontaktními údaji se v případě těchto karet rozumí: telefonní číslo, hlavní emailová adresa, webové
	stránky a \firstac{IČO}.
	Uživatel si pak vybere, jaké z těchto údajů bude chtít zveřejnit a jaké nikoli.

	\nlparagraph{Odkazy na profily na sociálních sítích}

	\noindent Pro většinu karet, zejména pak pro karty reprezentující soukromé osoby, budou nejdůležitější odkazy na sociální sítě.
	Pro jednoduché rozlišení mezi jednotlivými sítěmi a poskytnutí jistoty koncovým uživatelům, že daný odkaz skutečně
	...
	\nlparagraph{Generické odkazy na externí webové stránky}

	\noindent Generické odkazy budou podobné odkazům na sociální sítě.
	...
	\nlparagraph{Geografická lokace}

	\noindent Důležitou informací pro firmy, události, pobočky atd. je geografická lokace sídla.
	...
	\nlparagraph{Otevírací doba}

	\noindent Firmy a obchody většinou mají nějakou provozní/otevírací dobu, během níž jsou zaměstnanci schopni obsloužit své zákazníky.
	...
	\nlparagraph{Kanceláře, pobočky a zaměstnanci}

	\noindent Nedílnou součástí větších firem a institucí jsou pobočky a kanceláře rozmístěné v různých koutech světa.
	...
	Kromě těchto hlavních funkcí musí správná aplikace svým zaregistrovaným uživatelům poskytnout
	...
	S kradením identity uživatelů souvisí i další praktiky, jako jsou \firstac{CSRF} nebo kradení sezení uživatele.
...
webových stránek, aplikací či mikro služeb.
...
Nejrozšířenějším takovým kandidátem je programovací jazyk \ac{JS} dominující v současné době oběma kategoriím.
	...
	Společně s \firstac{UX} z velké části rozhodují, zda aplikace nebo webová prezentace
	...
	Je jich mnohem více a každým dnem vznikají nové, vývojářům značně ulehčují práci, a proto jsou tolik oblíbené.
		...
		Zástupci těchto technologií jsou \firstac{HTML}, \firstac{CSS}, \ac{JS} a nově i WebAssembly, a představují tedy základní
		...
		Struktura vychází z jazyka \firstac{XML}, který je obohacen hlavně
		...
		Nejrozšířenějším preprocesorem je jednoznačně \firstac{SASS}.
		...
		Méně používanou alternativou k \Ac{SASS} je \firstac{LESS}, který stejně jako \Ac{SASS} rozšiřuje základní \Ac{CSS}
		...
		\Ac{SASS} má např. pokročilejší možnosti scriptování zahrnující cykly a podmínky blížící se programovacím jazykům
		...
		Tyto komponenty představují samostatné bloky, jako jsou např. formuláře nebo tlačítka, udržující si vlastní
		data a při změně se automaticky překreslují.

		Momentálně nejznámější knihovnou pro tvorbu takových komponent je React, jenž
		byl vytvořen především pro tvorbu interaktivních \firstac{GUI} webových stránek a
		aplikací.
	...
	Pro komunikaci mezi těmito dvěma světy se používá standardizovaný aplikační protokol \firstac{HTTP}.
	...
	spojený.
		...
		Pro vývoj běžných webových stránek pak poskytuje nástroje pro implementaci \firstac{MVC}
		architektury, \firstac{REST} \firstac{API} architektury atp. \cite{spring_framework_documentaiton_web}
		...
		Dalším, i přes jeho stáří stále velmi oblíbeným jazykem je \firstac{PHP} zaměřující se především na tvorbu
		...
		Proto existují frameworky poskytující jistou úroveň abstrakce, čímž odstraňují nezbytnost tvorby základní logiky ze strany
		vývojáře,
...
% ############################
	...
	Existují dvě kategorie databázových systémů podle způsobu jakým pracují s daty: \firstac{SQL} a \firstac{NoSQL}.
	...
	Nicméně i přesto, že je tento jazyk standardizovaný, má každý databázový systém postavený na \Ac{SQL}
	vlastní implementaci a často obsahuje rozšíření nebo změny v syntaxi. \cite{sql_intro}
	...
	Nejrozšířenější jsou databáze pracující s \firstac{JSON} či \Ac{XML} dokumenty a databáze využívající strukturu klíč-hodnota.
	...
	Možnosti návrhu struktury dat jsou tak celkem rozsáhlé, bohužel pro aplikace využívající \firstac{OOP}
	...
	\Ac{NoSQL} databáze pracující s \Ac{JSON} dokumenty zjednodušující právě zmiňované komplikace při mapování
	...
	\cite{types-of-nosql-databases}
		...
		S tím souvisí množina vlastností zvaná \firstac{ACID}.
		...
		Jiná je také struktura dat pro optimální vyhledávání, které nemusí obsahovat nevyhledávaná data
	...
	varianty obrázků, poskytovat duplicitní úložiště napříč celým světem pro rychlé odezvy nazývané \firstac{CDN} a zálohování.
	...
	Hlavně když se ve webové aplikaci objeví nějaká nová událost, na kterou je potřeba uživatele upozornit
	okamžitě bez čekání.
	...
	Kromě upozorňovacích emailů, využívají webové aplikace čím dál častěji hromadné rozesílky novinek,
	...
	administrátoři.
	Dalším bezpečnostním prvkem sloužícím zejména pro bezpečí koncových uživatelů je \firstac{HTTPS} protokol zajišťující bezpečnou
	...
	Šifrování je zajištěno certifikátem \firstac{SSL} podepsaným věrohodnou autoritou.
	...
	To má obrovský dopad hlavně na zisky firmy stojící za aplikací, zejména pak pro internetové obchody.
		...
		Ten lze pronajmou celý nebo jen jeho část ve formě \firstac{VPS}.
		...
		Tento balíček je možné spustit na jakémkoli stroji podporujícím
		...
		průběžnou kontrolu, jestli se v aplikaci neděje něco neobvyklého.
		...
		servery ostatních společností zabývajících se čistě provozem serverů k pronájmu.
		...
		Lze si ale pronajmout již předkonfigurovaný server přímo pro konkrétní typ aplikace, v němž stačí navést poskytovatele
...
Následně je již možné začít se samotnou implementací s pomocí vybraných technologií a prototypu \ac{UI}.
	...
	nemusí být na první pohled úplně jednoznačné:

		\nlparagraph{Doména}

		\noindent Doména představuje objekty v systému, které zastupují reálný řešený problém.

		\nlparagraph{Business logika}

		\noindent Tento pojem označuje hlavní část zdrojového kódu zabývající se naplněním stanovených funkčních požadavků aplikace.
	...
	vhodných způsobů.
	Je tak na zvážení implementátora, jaký způsob je pro daný projekt vhodnější.
	...
	alternativám.
	Nástroje pro tvorbu šablon sice do jisté míry umožňují znovupoužívání částí stránek bez nutnosti duplicity,
	...
	Nicméně prohlížeč v tomto případě nemá přístup k datům, proto je nutné zajistit ještě nějaký zdroj dat a komunikaci
	...
	strukturovaných dat.
	Aplikace vystavující \ac{API} definuje, jaká data a funkce umí zpracovat, ostatní aplikace při dodržení těchto pravidel
	...
	Z pohledu vyhledávačů je zde nevýhoda omezených možností nastavení \firstac{SEO} vlastností z důvodu delegovaného sestavování
	stránek na prohlížeč.
	S tím má kvůli snížené podpoře \ac{JS} většina botů prohledávající weby problém.
	Naštěstí zvýšenou náročnost převyšují výhody v podobě velké dynamičnosti, přehlednosti a znovupoužitelnosti.
	...
	je na výběr mezi vzory \firstac{SOAP}, \ac{REST} nebo GraphQL \ac{API}.
	...
	GraphQL pomocí toho řeší problém se získáváním příliš mnoha dat nebo nedostatek dat, což je nevýhoda \ac{REST} \ac{API}.
	...
	projekt vyvíjí, a v tom, jestli má daný vývojář alespoň nějaké znalosti používaného programovacího či skriptovacího jazyka.
	...
	poměrně velkou částí samotného vývoje.
		...
		\subsubsection{Vybrané technologie}
			...
			\nlparagraph{Návrh UI a UX}

			\noindent Prvním nástrojem při návrhu aplikace by měl být grafický editor pro náčrt struktury a vzhledu.
			Vzhledem k tomu, že všechny dostupné nástroje obsahují podobné základní funkce, byl vybrán editor Figma díky
			schopnosti běhu v prohlížeči, čímž odpadá nutnost vlastnit zařízení s operačním systémem Windows nebo macOS.
			Program je navíc i pro základní kolaboraci mezi členy v týmu bezplatný.

			\nlparagraph{Technologie pro vývoj uživatelského prostředí}

			\noindent Vzhledem k tomu, že cílem je vytvořit webovou aplikaci, která poběží v standardních webových prohlížečích,
			...
			V případě \Ac{CSS} byl vybrán, pro jeho širokou komunitu a funkce, preprocesor \Ac{SASS}.
			Tento preprocesor umožní přehlednější kód a tím rychlejší vývoj.
			Kvůli výše zmíněné potřebě zobrazovat různé typy odkazů na kartách umožní \ac{SASS}
			do jisté míry automatizovat repetitivní a nepřehledné \ac{CSS} třídy pro velké množství barevných kombinací
			(každá sociální síť ve svém designu používá jiné barevné kombinace).
			...
			Alternativou může být jazyk C\# s technologií Blazor, díky které je možné se od ekosystému
			\ac{JS} úplně odstínit.
			...
			\nlparagraph{Technologie pro vývoj API serveru}

			\noindent Co se týče technologií pro vývoj serverové aplikace fungující jako \ac{API} poskytující data, je výběr spíše
			...
			Spring.
			Kromě podpory pro \ac{REST} \ac{API}, přichází ekosystém Spring například s vlastní knihovnou pro
			zabezpečení dat.

			\nlparagraph{Databázový systém jako zdroj dat}

			\noindent Pro uchovávání uživatelských dat je nutné vybrat vhodný databázový systém.
			...
			Z bezplatných databází by se dala vybrat například PostgreSQL, MySQL nebo MariaDB, všechny by byly schopny
			poskytnout potřebné funkce pro vývoj navrhovaného řešení.
			Finální výběr byl spíše preferencí databázového systému PostgreSQL.

			\nlparagraph{Databázový systém pro fulltextové vyhledávání}

			\noindent U systému pro vyhledávání dat je rozhodování celkem jednoduché, avšak jedná se o preferenci, protože
			...
			\nlparagraph{Souborové úložiště}

			\noindent Dalším důležitým úložištěm dat je úložiště uživatelských souborů, v tomto případě se jedná především o obrázky.
			...
			\nlparagraph{Transakční emaily a rozesílky}

			\noindent Protože bude aplikace pracovat se zaregistrovanými uživateli, bude potřeba uživatelům odesílat transakční emaily
			...
			odesílání pomocí knihovny JavaMail, která se stará pouze o odeslání emailů.
			...
			s již připravenou infrastrukturou pro tvorbu šablon a jednoduchou možností nastavit odesílání emailů tak, aby
			...
			se musí platit.
			Avšak tato řešení přináší záruku určité spolehlivosti doručitelnosti emailů, a to díky serverům s dobrou pověstí
			...
			\nlparagraph{Provoz aplikace}

			\noindent Poslední částí skládanky je samotný provoz v několika prostředích.
			...
			Pro tento konkrétní projekt byla vybrána služba Checkly z důvodu poměrně široké nabídce funkcí, zejména pak
			možnosti monitorovat dotazy na webové stránky, dotazy na \ac{API}, expiraci SSL certifikátu a možnosti napojení na
			komunikační aplikace (např.: Discord).

	\subsection{Prototyp}

	Prvotní verze aplikace byla postavena na frameworku Spring společně \firstac{ORM} frameworkem Hibernate pro
	...
	Nicméně i přes to, že se jednalo o ranou fázi aplikace s minimem funkcí, začalo být postupně znát, že vytvořit
	...
	od nuly s analýzou technologií a návrhových architektur.
	Část původního kódu se sice použila znovu, nicméně původní kód byl po analýze natolik přepracován, že se dá uvažovat
	...
	Tento jazyk má zajistit jednotu designu, napříč celou aplikací, pro poskytnutí intuitivnějšího rozhraní a zamezení
	frustrace uživatelů.
	...
	I tvůrci Material Designu berou tyto materiály jako výchozí bod a počítají s tím, že si ho daný projekt přizpůsobí
	...
	mezer a prvků a základní pravidla, jako je pozicování tlačítek, rozvržení seznamů, modální okna a podobně.
		...
		Mezi hlavní pravidla patří nezatěžování stránek příliš mnoha informacemi, které mnohdy ani nejsou potřebné.
		...
		Neméně důležité je vizuálně navést uživatele k tomu, co na dané stránce může vše provádět.
		...
		Rozvržení prvků ale není vše, je potřeba myslet i na samotný design.

		\nlparagraph{Paleta barev}

		\noindent Důležitou částí designu je správně připravená paleta barev, jenž bude použita napříč aplikací.
		Ta definuje jak obecné barvy prvků, jako jsou texty, bloky, tlačítka, tak i ty méně obecné.

		\cntcapfigure{paleta_barev}{12cm}{Navržená paleta barev.}{[autor]}

		\nlparagraph{Stíny a linky}

		\noindent Dalším velmi důležitým designovým prvkem jsou stíny pomáhající vizualizovat pomyslnou vzdálenost jednotlivých
		...
		Pro tento projekt byl vybrán způsob, ve kterém se stín snaží simulovat reální stín generovaný světelným zdrojem.
		...
		\cntcapfigure{ukazka_stinu_a_linek}{8cm}{Ukázka kombinace stínů a linek v jednom bloku.}{[autor]}

		\nlparagraph{Okraje}

		\noindent Kromě linek a stínů je dobré myslet i na samotné okraje prvků, tedy jestli budou mít zaoblené rohy, či nikoli.
		Po vzoru Material Designu byly pro větší lehkost prvků zvoleny zaoblené rohy o průměru 8px.
		Toto rozhodnutí je však celkem subjektivní a někteří uživatelé mohou preferovat ostré rohy.

		\nlparagraph{Signalizace výběru}

		\noindent Posledním pravidlem je zobrazování signalizace výběru.
		Tato část musí být pro koncové uživatele napříč aplikací obzvlášť konzistentní a intuitivní.
		...
		Varianta pro pás prvků se použije v případě pásu prvků, u kterých nemůže dojít k přetečení na více řádcích.
		...
		\cntcapfigure{ukazka_vyberu_1}{4cm}{Ukázka signalizace výběru v navigaci.}{[autor]}

		\cntcapfigure{ukazka_vyberu_2}{3cm}{Ukázka signalizace výběru konkrétního prvku.}{[autor]}

		\newpage

		\subsubsection{Strukturální prvky}

		Strukturálními prvky jsou myšleny takové prvky, které slouží jako základní stavební kameny pro jednotné
		rozvržení stránek.

			\nlparagraph{Blok obsahu}

			\noindent Stránky jsou rozvrženy hlavně pomocí tzv. bloků obsahu označujících jeden logický celek informací
			na stránce.
			...
			\cntcapfigure{blok_obsahu_ukazka}{\linewidth}{Ukázka využití bloku obsahu.}{[autor]}

			\nlparagraph{Lišta akcí}

			\noindent Kromě hlavní navigační lišty aplikace bylo nutné zavést ještě jeden typ lišty - lištu akcí.
			Díky ní je možné na určitých stránkách zobrazit sekundární lištu s nadpisem a dodatečnými akcemi, jež se stahují
			...
			\cntcapfigure{lista_akci}{\linewidth}{Ukázka lišty akcí.}{[autor]}

			\nlparagraph{Modální okno}

			\noindent Modální okna představují bloky obsahu, které pro zdůraznění
			...
			Informační okno patičku neobsahuje a slouží k zobrazení již existujících dat a nevyžaduje od uživatele žádnou akci.
			Lze ho zavřít buď křížkem v hlavičce, nebo kliknutím mimo okno.
			Akční okno akci vyžaduje a uživatel musí vždy nějakou zvolit, okno nelze zavřít pouhým kliknutím mimo okno.
			...
			\cntcapfigure{modalni_okno_informacni_ukazka}{12cm}{Ukázka informačního modální okna.}{[autor]}

			\cntcapfigure{modalni_okno_akcni_ukazka}{12cm}{Ukázka akčního modálního okna.}{[autor]}

			\newpage

			\nlparagraph{Kontextuální nabídka}

			\noindent Jedná se o malé menu zobrazující rozšířené možnosti nějaké akce.
			Většinou se používá, pokud není dostatek
			místa v \ac{UI} pro všechny dostupné akce, nebo ve formuláři pro výběr konkrétní položky z výběrového pole.

			\cntcapfigure{kontextualni_nabidka_seznam_ukazka}{8cm}{Ukázka kontextuální nabídky akcí položky v seznamu.}{[autor]}
		...
		Po prvcích strukturující obsah je možné navrhnout prvky, se kterými bude uživatel přímo či nepřímo interagovat.
			...
			\noindent Nejhlavnějším takovým prvkem je tlačítko.
			...
			Všechna tlačítka byla rozdělena na tlačítka s nízkou prioritou, vysokou prioritou a vysokou vážností akce.

			Tlačítko s nízkou prioritou reprezentuje vedlejší akci, kterou sice uživatel může provést, ale není jeho hlavním cílem.
			Takové tlačítko má neutrální barvu a ve většině případů neobsahuje ani ikonu.

			\cntcapfigure{tlacitko_s_malou_prioritou}{4cm}{Ukázka tlačítka s malou prioritou.}{[autor]}
			...
			\cntcapfigure{tlacitko_s_velkou_prioritou}{4cm}{Ukázka tlačítka s vysokou prioritou.}{[autor]}

			Pro případy, v nichž uživatel provádí nějakou akci s vysokou vážností, jako je například nevratné smazání nějakého
			objektu, vzniklo rozšíření tlačítka s vysokou prioritou.
			Tlačítko se liší pouze červenou barvou evokující dojem závažné akce.

			\cntcapfigure{tlacitko_s_velkou_prioritou_a_velkou_vaznosti}{4cm}{Ukázka tlačítka provádějící nevratnou změnu.}{[autor]}

			\nlparagraph{Vstupní pole}

			\noindent Vstupní pole různých druhů jsou společně s tlačítky nejdůležitějšími prvky jakékoliv aplikace.
			Vstupní pole umožňují uživatelům předat data aplikaci v různých formátech.
			Vzniklo proto několik základních typů vstupních polí, které lze uvnitř formulářů jednoduše kombinovat.
			...
			\cntcapfigure{textove_pole}{10cm}{Ukázka textového pole s ikonou a nápovědou.}{[autor]}

			\newpage

			Extenzí textového pole jsou pole pro zadání času a data.
			Ta rozšiřují manuální zadávání o okénka pro pohodlný výběr bez nutnosti klávesnice.

			\cntcapfigure{datumove_pole}{6cm}{Ukázka pole s výběrem data a času.}{[autor]}
			...
			\cntcapfigure{pole_lokace}{10cm}{Ukázka pole pro výběr geografického bodu v mapě.}{[autor]}

			\newpage

			Posledním méně používaným polem v této aplikaci je pole pro výběr obrázku.
			Pole umožní výběr souboru ze souborového systému uživatele, který je následně nahrán a zobrazen jeho náhled
			přímo v poli.
			Vybraný obrázek je pak možné jednoduše změnit nebo úplně odstranit.

			\cntcapfigure{obrazkove_pole}{4cm}{Ukázka pole s nahraným obrázkem.}{[autor]}

			\nlparagraph{Seznam položek}

			\noindent Velmi využívaným prvkem je seznam položek.
			...
			\cntcapfigure{seznam}{\linewidth}{Ukázka pokročilého seznamu prvků.}{[autor]}

			\nlparagraph{Stránkování}

			\noindent Při velkém množství položek v jednom seznamu se využívá systém stránkování.
			Prvek stránkování umožňuje přepínat mezi konkrétními stránkami nebo se přepínat mezi předchozí a následující stránkou.

			\cntcapfigure{strankovani}{4cm}{Ukázka stránkování.}{[autor]}
		...
		\cntcapfigure{cely_navrh}{\linewidth}{Finální grafický návrh aplikačních stránek a prvků.}{[autor]}
	...
	\subsection{Implementace API serveru}
		...
		Stěžejní je to z důvodu budoucí čitelnosti kódu a možností rozšíření o nové funkce.
		...
		\nlparagraph{Model uživatelských účtů}

		\noindent Modul uživatelských účtů je zodpovědný za vytváření, uchování, úpravu a mazání jednotlivých účtů.
		...
		\cntcapfigure{datovy_model_ucet}{\linewidth}{Datový model popisující uživatelské účty.}{[autor]}

		Jak je možno vidět z diagramu, účet má kromě základních přihlašovacích informací i spoustu
		doplňujících dat.

		V první řadě má každý účet přiřazený vždy nějaký stav.
		...
		Další velmi důležitou komponentou je systém ověřovacích tokenů sloužících k potvrzování akcí, u nichž
		je potřeba ověřit, zda akci provádí skutečný uživatel, nikoliv podvodník.
		...
		Ten vzniká aktivací prémiového plánu a říká, že účet má práva na prémiové funkce.
		V jistých případech může mít přiřazený i datum expirace, se kterým automaticky zanikne.
		S tím se váže i stav expirace, který pomáhá upozorňovat uživatele na současný stav předplatného.

		\nlparagraph{Model kupónů}

		\noindent Předplatné může vzniknout na základě zadání platného kupónu, avšak systém kupónů je navržen obecně, aby
		bylo možné ho v případě budoucích rozšíření jednoduše použít znovu i na aktivaci jiných funkcionalit, nejen
		pro prémiové plány.
		...
		\cntcapfigure{datovy_model_kupon}{6cm}{Datový model kupónového systému.}{[autor]}

		\nlparagraph{Model karet}

		\noindent Nejhlavnějším prvkem v této aplikaci jsou samotné karty a jejich data.

		\cntcapfigure{datovy_model_karta}{\linewidth}{Datový model karet a navázaných dat.}{[autor]}
		...
		Typ karty definuje, jak lze s danou kartou pracovat a jaká data poskytuje.
		...
		Právě kvůli těmto nevýhodám a žádným jasným výhodám byly tyto dva typy, především pro pohodlí
		koncových uživatelů, sjednoceny.
		...
		Tato data reprezentují soubor údajů, který daný typ karty podporuje.
		...
		Popisek slouží především pro odlišení více odkazů vedoucích na stejnou webovou stránku.
		\ac{URL} adresa je hlavní informace a z důvodu bezpečnosti musí být validována oproti zvolené šabloně sítě.
		Díky tomu není možné maskovat podvodnou síť důvěryhodnou.
		Jednotlivé odkazy jsou pak seskupovány do skupin odkazů vycházejících ze stejné šablony.
		I přes to, že skupiny zastřešují vždy nějaký soubor sítí stejného typu, může existovat více skupin vycházejících ze
		stejné šablony.
		...
		a je tak nutné definovat všechny, a protože není jasné, jakou \ac{URL} adresu uživatel zadá.
		...
		Proto každá obecná karta může mít libovolné množství přiřazených poboček/kanceláří/prodejen, z nichž má každá své jméno
		...
		Každý den má pak definovaný interval označující, kdy má daná entita otevřeno a kdy má přestávku.

		\newpage

		\nlparagraph{Model oblíbených karet}

		\noindent Oblíbené karty umožňují k uživatelskému účtu přiřazovat cizí karty jako oblíbené.
		...
		Každá oblíbená karta může mít navíc jednu textovou poznámku s formátováním.
		...
		\cntcapfigure{datovy_model_oblibene_karty}{\linewidth}{Datový model oblíbených karet uživatelských účtů.}{[autor]}

		\subsubsection{Práce s datovým modelem}

		Práci s datovým modelem zastřešuje právě již několikrát zmíněná business logika.
		Struktura business logiky si bere inspiraci z návrhového vzoru \firstac{DDD}, který se zabývá
		...
		Jednotlivé služby by se měly vždy zabývat pouze jedním typem dat kvůli přehlednosti a ostatní data delegovat na
		...
		Konzumující logika si pak může přesně vybrat, s jakými daty chce pracovat.
		...
		\cntcapfigure{sluzby_a_repozitare}{\linewidth}{Zjednodušený model služeb a repozitářů.}{[autor]}
		...
		Bohužel se při postupném rozšiřování aplikačních požadavků ukázalo, že rozpad logických celků entit není úplně
		...
		objektů.
		Složitost je hlavně v načítání a ukládání komplexních entit složených z více objektů a kolekcí, hlavně pak
		kolekcí 1:N (případně M:N).
		Jazyk \ac{SQL} totiž neumožňuje jednoduše vrátit více řádků z různých tabulek, a tak není možné objekty různých
		tříd tvořící jeden celek najednou vrátit.
		...
		Poměrně novou zajímavou alternativou je Spring Data JDBC, bohužel v době vývoje nebyla tato knihovna příliš
		rozšířená a tak osvědčená, aby se dostala do výběru.
		...
		Díky tomu je Hibernate schopen poskytovat mezipamět pro entity při vícenásobném dotahování, při ukládání
		...
		zkušených programátorů využívajících Hibernate spoustu let, kteří si nemyslí, že je Hibernate ideálním nástrojem
		...
		v \ac{API} knihovny.
		Na druhou stranu si spousta uživatelů míru abstrakce pochvaluje. \cite{bad_hibernate}

		Při výběru tedy hodně záleží na konkrétním projektu a zkušenostech implementátora.
		Pro tuto aplikaci byl zpočátku zvolen Hibernate, právě díky jeho abstrakci mapování.
		...
		Za cenu prvotní transformace zjednodušil MyBatis práci s entitami a hledání chyb bylo přímočařejší.
		...
		Ten se ukázal jako dostačující, avšak našly se výjimky, kdy složitost byla použitého \ac{SQL} kódu
		na hraně přehlednosti.
...
public interface AccountDao {
  ...
  Optional<Account> findByEmail(String email);
			...
			\end{verbatim}
			\captionsource{Ukázka získání uživatelského účtu z databáze MyBatisem.}{[autor]}
		\end{codeblock}

		\subsubsection{Validace entit}

		Validace entit je velice důležitá, pokud mohou uživatelé data upravovat zvenčí.
		Díky ní se do databáze dostanou jen validní data.
		Tato aplikace využívá dvou typů validací a každou pro validování něčeho jiného.
		...
		Příkladem může být stav karty, v němž nesmí smazaná karta již přejít do stavu aktivní karty, protože nemá potřebná data.
...
public class CardLinkGroup {
  ...
  private List<@Valid CardLink> links;
			...
			\end{verbatim}
			\captionsource{Ukázka třídy s validačními pravidly pomocí Java anotací.}{[autor]}
		\end{codeblock}

		Díky rozšiřitelnosti této knihovny je možné jednoduše implementovat i druhý typ validací.
		...
		Cloudinary poskytuje jeden velký adresář pro všechny soubory, což je při více typech souborů nevhodné.
...
public class StorageFile {
  ...
  private final String url;
			...
			\end{verbatim}
			\captionsource{Třída popisující uložený soubor v Cloudinary.}{[autor]}
		\end{codeblock}
...
https://res.cloudinary.com/id_uloziste/image/upload/t_256x256/id_souboru
			\end{verbatim}
			\captionsource{Ukázka URL adresy souboru v Cloudinary úložišti.}{[autor]}
		\end{codeblock}
		...
		Tím, že aplikace bude využívat externí službu SendinBlue, bylo v aplikaci nutné připravit pouze propojení
		...
		podpora zobrazování stylovaného \ac{HTML} obsahu v
		...
		Události jsou v tomto případě reprezentovány aplikačními událostmi z frameworku Spring.
...
public class CardUpdatedEvent extends ApplicationEvent {
  ...
  /* ... */
			...
			\end{verbatim}
			\captionsource{Ukázka aplikační události.}{[autor]}
		\end{codeblock}

		Příkladem může být událost vyvolaná jako následek uložení upravené karty.
		Událost reprezentuje úpravu karty a nese s sebou upravenou kartu, kterou může pozorovatel
...
public class CardUpdatedEventListener {
  ...
  public void processCardUpdatedEvent(@NonNull CardUpdatedEvent event) {
    /* ... */
		...
		Modul má do systému vystavené na konkrétní aplikační události vlastní posluchače (změna emailu uživatele, změna hesla,
		vypršení platnosti předplatného...), kteří při vzniku událostí odesílají pomocí \ac{API} emaily.
		...
		Fulltextové vyhledávání pracuje s tzv. analyzátory, které analyzují text a generují z něj index relevantních
		slov.
		...
		Poté je takto upravený text předán tokenizérům, které text rozdělí na kolekci tokenů.
		Token může představovat cokoli, co dává v kontextu aplikace smysl, nicméně většinou přestavuje jedno
		slovo.
		...
		Pro tuto aplikaci byl sestaven analyzér skládající se primárně z tokenizéru rozdělujícího věty na slova a filtru
		...
		Kromě obecných konfiguračních hodnot obsahuje hlavně strukturu dokumentů, aby byla databáze schopna efektivně vyhledávat.
		...
		Specifikované analyzéry by měly odpovídat analyzéru použitému při vyhledávání.
...
final IndexResponse cardResponse = esClient.index(i -> i
    ...
    .document(cardToIndex));
			\end{verbatim}
			\captionsource{Indexace jedné karty do Elasticsearch databáze.}{[autor]}
		\end{codeblock}

		Hlavní řešené problémy spočívaly v přípravě entit a správě indexů.
		Tuto problematiku totiž Elasticsearch knihovna řeší ve formě dílčích operací, nikoli však jako celek.
		...
		V případě karet je nezbytné komplexní strukturu dat značně zjednodušit, protože by jinak Elasticsearch
...
public class SearchableCard {
  ...
  /* ... */
			...
			\end{verbatim}
			\captionsource{Indexovatelná karta. Obsahuje metadata a vyhledatelná data.}{[autor]}
		\end{codeblock}
...
jan_novak
			\end{verbatim}
			\captionsource{Ukázka transformace URL adresy na profilové ID.}{[autor]}
		\end{codeblock}
		...
		\cntcapfigure{proces_reindexace_na_urovni_es}{5cm}{Proces hromadné reindexace dat na úrovni Elasticsearch.}{[autor]}
		...
		\cntcapfigure{proces_reindexace_z_db}{5cm}{Proces hromadné reindexace nových dat z primární databáze.}{[autor]}
		...
		\firstac{YAML} jazykem, jenž umožňuje lépe definovat celé objekty nastavení.
...
accounts:
  ...
  subscription:
    process-expiration-every: 60
			\end{verbatim}
			\captionsource{Ukázka části nastavení aplikace v YAML konfiguračním souboru.}{[autor]}
		\end{codeblock}
		...
		\cntcapfigure{datovy_model_nastaveni_aplikace}{\linewidth}{Datového model nastavení aplikace.}{[autor]}
		...
		Definice šablon jsou načítány z \ac{YAML} souboru v rámci celého nastavení aplikace.
			...
			\nlparagraph{Šablony URL adres}

			\noindent Šablony \ac{URL} adres slouží pro specifikování vzoru povolených \ac{URL} adres dané sítě.
			...
			Regex je výrazový jazyk skládající se ze speciálních znaků definujících pravidla porovnávání \cite{regex}.
...
https://{userPart}.example.com
				\end{verbatim}
				\captionsource{Ukázka šablon URL adres.}{[autor]}
			\end{codeblock}

			Systém obsahuje různé validace aplikované při analyzování šablon, aby se do aplikace nemohly načíst
			nevalidní šablony.
			...
			\nlparagraph{Validace odkazů}

			\noindent Standardní validace se skládá ze dvou hlavních částí.
			...
			V případě vyžadované validace existence přechází validátor do druhé části.
			...
			existuje, neexistuje, nebo \ac{URL} adresa přesměrovává na jinou stránku.
			...
			\cntcapfigure{proces_validace_odkazu}{\linewidth}{Proces validace URL adresy odkazu.}{[autor]}
			...
			U některých stránek to skutečně pomohlo, jiné jsou ale pečlivější.
		...
		obsahujícího datum expirace.
		Datum expirace je vypočítáno z doby platnosti kupónu.
		Každý kupón definuje, po jakou dobu bude předplatné aktivní; při aktivaci se tato doba přičte k aktuálnímu datu.
		V tuto chvíli uživatel může využívat všechny prémiové funkce.
		Pokud by v tuto chvíli uživatel aplikoval další kupón, datum expirace by se změnilo podle nového kupónu.

		V případě placeného předplatného by objekt předplatného zprvu datum expirace neměl.
		To by se nastavilo automaticky až při neprovedené následující platbě.

		Systém tedy při vytváření a ukládání karet musí vždy validovat zda daná karta neobsahuje prémiový obsah.
		Pokud ano, musí zkontrolovat, zdali má uživatel příslušnou roli.
		...
		Tou nejjednodušší je upozornění ignorovat - v takovém případě se po nějaké době všechny karty uživatele skryjí
		...
		K zajištění komunikace mezi front-endovou \ac{GUI} aplikací a aplikací \ac{API} serveru, vystavuje server
		právě \ac{REST} \ac{API}.
		...
		mapovat na \ac{JSON} dokumenty. \cite{restfulapi}
			...
			\nlparagraph{Struktura API}

			\noindent Implementované \ac{API} v běžných scénářích (získání, vytvoření a úprava zdrojů) respektuje doporučovaná pravidla.
			...
			\cntcapfigure{api_model_ucty}{\linewidth}{Stromový přehled struktury API uživatelských účtů.}{[autor]}
			...
			\cntcapfigure{api_model_karty}{\linewidth}{Stromový přehled struktury API karet.}{[autor]}

			\ac{API} úložiště momentálně umožňuje soubory pouze nahrávat, protože systém pracuje pouze s veřejnými soubory, které
			lze získat přímo z Cloudinary serverů.

			\cntcapfigure{api_model_uloziste}{10cm}{Stromový přehled struktury API uložiště.}{[autor]}

			Pro přehlednost vyhledávání karet a míst existuje separátní vyhledávací \ac{API}.

			\cntcapfigure{api_model_vyhledavani}{10cm}{Stromový přehled struktury API vyhledávání karet.}{[autor]}

			Aplikace také nabízí interní administraci pro správce.
			Proto vzniklo i \ac{API} pro spouštění servisních operací a ke správě zákulisních zdrojů (např.: kupóny).

			\cntcapfigure{api_model_adm}{\linewidth}{Stromový přehled struktury API operací administrace.}{[autor]}

			\nlparagraph{Objekty pro přenos dat}

			\noindent V rámci komunikace mezi konzumentem \ac{API} a serverem je nezbytné přenášet objekty reprezentující data
			aplikace.
			...
			Právě z těchto důvodů \ac{API} využívá tzv. \firstac{DTO}.
			Tyto datové objekty slouží obecně pro přenos dat mezi procesy.
			...
			Speciálním případem jsou \ac{DTO} objekty, jež slouží pouze pro jednorázové předání dat.
...
public interface DtoFactory<E, D> {
  ...
  E updateEntity(E originalEntity, D dto);
				...
				\end{verbatim}
				\captionsource{Obecné rozhraní třídy pro transformaci mezi doménovými a DTO objekty.}{[autor]}
			\end{codeblock}
			...
			Rozhraní továrny je generické, a proto má každá doménová entita vlastní implementaci.

			\nlparagraph{Zabezpečení}

			\noindent Zabezpečení je velmi důležitou součástí aplikace pro znemožnění zneužití dat útočníky.
...
protected void configure(HttpSecurity http) throws Exception {
  http
      ...
      /* ... */
				...
				\end{verbatim}
				\captionsource{Ukázka části nastavení zabezpečení API pomocí knihovny Spring Security.}{[autor]}
			\end{codeblock}

			\nlparagraph{Přihlašování}

			\noindent Korektní implementace procesu přihlašování je pro aplikaci využívající uživatelské účty stěžejní.
			...
			Tento token je následně předán klientovi, který musí zaručit předání vygenerovaného tokenu severu.
			...
			To lze řešit například ukládáním tokenů do databáze, nicméně správa tokenů se pak částečně přenáší zpět
			...
			Díky tomu se celý proces zjednodušil a delegoval na externí knihovnu, na které pracují experti z oboru.
			...
			\cntcapfigure{proces_prihlaseni_oauth2}{\linewidth}{Proces zpracování přihlášeného OAuth2 uživatele a transformace na interního uživatele.}{[autor]}
...
public void getMostUsedTags_ExpectedFoundCorrectOnes() {
  ...
  assertEquals(
      ...
      cardTagService.getMostUsedTags(3)
			...
			\end{verbatim}
			\captionsource{Testu ověřující, že služba štítků vrací správné nejvyužívanější štítky z nasimulovaného repozitáře.}{[autor]}
		\end{codeblock}
		...
		Ke spuštění takových testů je tedy potřeba běžící testovací databáze s připravenými testovacími daty.
...
public void validateLink_InvalidLink_ExpectedFalse() {
  ...
  mockMvc.perform(post("/api/cards/links/validate")
      ...
      .andExpect(content().contentType(MediaType.APPLICATION_JSON));
			...
			\end{verbatim}
			\captionsource{Ukázka testu volání API validující odkaz.}{[autor]}
		\end{codeblock}
	...
	Zaměření tak padlo na konkrétní problémy řešené domény.
		...
		Opět existuje spousta ověřených knihoven řešících problémy komunikace.
...
$http.onRequest(async (request) => {
  if (!csrfAllowedMethods.includes(request.method)) {
    await addCsrfTokenToRequest(store, $http, request)
			...
			\end{verbatim}
			\captionsource{Vkládání CSRF tokenu do každého požadavku putujícího na API server.}{[autor]}
		\end{codeblock}
		...
		Lokalizace textů se zabývá zobrazováním textů prvků \ac{UI} v konkrétním jazyku uživatele.
		...
		\cntcapfigure{nevalidni_pole_formulare}{8cm}{Ukázka pole formuláře s textem v nevalidním formátu.}{[autor]}

		Pomocí knihovny Vuelidate bylo implementováno zobrazování chyb přímo v konkrétních polích, hned
		jak uživatel vyplní dané pole.
		...
		Toho bylo využito především u validace \ac{URL} adres odkazů na sociální sítě, u nichž je potřeba se dotázat \ac{API},
...
return async function (displayId) {
  ...
  try {
    ...
    this.$notifyError()
			...
			\end{verbatim}
			\captionsource{Vlastního validátor využívající API k validaci hodnoty.}{[autor]}
		\end{codeblock}
		...
		systémů.
		Tento způsob je výhodný v tom, že si uživatel upozornění všimne, ať už je kdekoliv na stránce, i
		při přechodu mezi modálními okny.

		\cntcapfigure{upozorneni_chyba}{6cm}{Ukázka upozornění na chybu vzniklou špatnými přihlašovacími údaji.}{[autor]}

		K implementaci upozornění byla využita knihovna Vue.js notifications
		poskytující oproti ostatním podobným knihovnám dostatečnou volnost přizpůsobení vzhledu a chování.
		...
		Malou výjimkou jsou chybové zprávy, v nichž existují dvě metody: jedna s možností vložení vlastního textu, druhá
...
this.$notifyError()
			\end{verbatim}
			\captionsource{Pomocné metody pro vystavení různých typů upozornění.}{[autor]}
		\end{codeblock}
...
ENTRYPOINT yarn start --dotenv=.env.$ENV
			\end{verbatim}
			\captionsource{Dockerfile spouštějící front-endovou aplikaci v Dockeru.}{[autor]}
		\end{codeblock}

		\subsubsection{Nastavení Kubernetes}

		Na základě připravených Docker images byl pro obě aplikace zprovozněn systém Kubernetes.
		...
		\cntcapfigure{architektura_produkce}{\linewidth}{Putování HTTP požadavku infrastrukturou.}{[autor]}

		Webový server slouží jako vstupní brána do celého systému a poskytuje nástroje pro zabezpečení požadavků.
		Zároveň zahazuje neplatné požadavky ještě dřív, než by mohly způsobit škody v samotných aplikacích.
		...
		Deploymenty se používají v případě, že aplikace neudržuje důležitá data, o která by se v případě vypnutí přišlo
		\cite{deployements}.
...
Kvůli velmi podobným výhodám napříč programovacími jazyky byl vybrán jazyk Java společně s frameworkem Spring, ten ale
spíše podle osobních preferencích a předchozích znalostí.

Další rozhodování se zabývalo výběrem vhodné primární databáze pro uchování uživatelských dat a nástroje pro
...
Nakonec byl pro provoz aplikace zvolen systém Kubernetes a poskytovatel DigitalOcean.
...
kódu aplikace.
Na základě modelu byla nejdříve implementována business logika, jež byla vzápětí obalena \ac{REST} \ac{API}.
...
Nakonec byly pro obě aplikace vytvořeny Docker image a nakonfigurován systém Kubernetes.
...
% todo
% - změnit vzhled FE
% - přechody změnit na fialovou všude i tady
...
% - smazat testy které neprocházejí
% - oauth2 provideři
% - smazat security.txt a klíče
