...
Následně je již možné začít se samotnou implementací s pomocí vybraných technologií a prototypu \ac{UI}.
	...
	\subsection{Implementace API serveru}
		...
		v \ac{API} knihovny.
		Na druhou stranu spousta uživatelů této knihovny jsou spokojeni s odstínením \ac{SQL} jazyka a problémů jako
		...
		Avšak pro tuto aplikaci bylo rozhodnutí jednoznačné zejména kvůli výše zmíněné validaci verzí entit.
		Právě proto byla zvolena knihovna MyBatis společně s knihovnou zjednodušující nastavení díky napojení na
		framework Spring.
...
public class CardLinkGroup {
	...
	/* ... zbytek kódu byl vynechán pro přehlednost ... */
		...
		\end{lstlisting}
		Tato třída obsahuje vlastní třídní pravidlo pro validaci URL adres vnořených odkazů na základě definované šablony,
...
return Optional.of(new StorageFile(
	...
	(String) response.get("secure_url")
		...
		Cloudinary pomocí speciální URL adresy obsahující identifikaci úložiště, identifikaci konkrétního souboru a
		požadovanou variantu.
		Díky přímému přístupu je možné využít \ac{CDN} pro menší odezvy při stahování.
		Konkrétní URL adresa pro stažení profilové obrázku karty v podobě varianty o rozměrech 256x256 pixelů může
		vypadat následovně:
		\begin{lstlisting}
https://res.cloudinary.com/id_uloziste/image/upload/t_256x256/id_souboru
		\end{lstlisting}
		V tomto případě je varianta nakonfigurována přímo v administraci Cloudinary a v URL adrese je pouze její jméno,
		je však možné formát varianty nadefinovat přímo v URL adrese.
...
public void processAccountNewEmailRequestedEvent(@NonNull AccountNewEmailRequestedEvent event) {
	...
	mailService.sendTransactionalEmail(event.getNewEmail(), EmailTemplate.ACCOUNT_NEW_EMAIL, templateArguments);
		...
		\subsubsection{Vyhledávání karet}
		...
		TODO - problém se změnou mapingu, translace entity na searchable entity, více typů indexů ale jedno vyhledávání

		\subsubsection{Šablony odkazů sítí}


		 todo stránkování?, search, , app config, plány, generování bezpečných tokenů?
